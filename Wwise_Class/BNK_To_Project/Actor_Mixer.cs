...
namespace WoTB_Voice_Mod_Creater.Wwise_Class.BNK_To_Wwise_Project
{
    ...
    public class Get_Config
    {
        ...
        private static readonly FNV_Hash_Class hasher = new FNV_Hash_Class();
        //value=の後の数字を取得
    ...
    public class Actor_Mixer
    {
        ...
        private int Property_End_Line = -1;
        private readonly List<uint> RTPC_ShortID = new List<uint>();
        private readonly List<uint> Master_ShortID = new List<uint>();
        private readonly List<uint> Switch_Child_ShortID = new List<uint>();
        private readonly List<uint> Switch_Parent_ShortID = new List<uint>();
        private System.Windows.Controls.TextBlock Message_T;
        public Actor_Mixer()
        {
        ...
        {
            Message_T = TextBlock;
            Message_T.Text = "Creating 'Actor_Mixer'...";
            await Task.Delay(50);
            //全プロジェクト共通の記述
                    ...
                    foreach (int Number in Not_Done_Number)
                    {
                        ...
                        foreach (Actor_Mixer_Info Temp_Relation in BNK_Info.Actor_Mixer_Child_Line)
                        {
                            ...
                            if (BNK_Info.Relation[Number].Parent == Temp_Relation.ShortID)
                            {
                                ...
                                _ = Not_Done_Number.Remove(Number);
                                IsOK = true;
                                IsExist = true;
                                break;
                            }
            ...
            if (!Directory.Exists(Path.GetDirectoryName(To_File)))
                _ = Directory.CreateDirectory(Path.GetDirectoryName(To_File));
            ...
            Message_T.Text = "Created 'Actor_Mixer' file.";
            await Task.Delay(50);
            //RTPCのShortIDを文字列に変換
            if (RTPC_ShortID.Count > 0)
            {
                foreach (uint ShortID_Now in RTPC_ShortID)
                {
                    ...
                    Message_T.Text = "Converting '" + ShortID_Now + "' to a string...";
                    await Task.Delay(50);
            ...
            if (Master_ShortID.Count > 0)
            {
                ...
                Message_T.Text = "Adapted 'Master Audio Bus'.";
                await Task.Delay(50);
            }
        ...
        private async Task Set_Switch_Name()
        {
            ...
            if (Switch_Child_ShortID.Count > 0)
            {
                ...
                Message_T.Text = "Adapted 'Switch'.";
                await Task.Delay(50);
            }
        }
        //Get_Actor_Mixer_Historyから呼ばれます
        private void Set_Actor_Mixer_Container(BNK_Relation Temp_Relation, bool IsExtractSound)
        {
            int From_Line = Temp_Relation.Line;
            int To_Line;
            int Temp_Line = Temp_Relation.Line;
            uint OverWriteParent = Temp_Relation.Parent;
            bool IsNoParent = Temp_Relation.Parent == 0;
            while (true)
            {
                Temp_Line++;
        ...
        //Blend Containerを設定
        private void Add_CAkLayerCntr(BNK_Relation Temp_Relation, int From_Line, int To_Line)
        {
        ...
        private void Add_CAkActorCntr(BNK_Relation Temp_Relation, int From_Line, int To_Line)
        {
        ...
        //Random Containerを設定
        private void Add_CAkRanSeqCntr(BNK_Relation Temp_Relation, int From_Line, int To_Line)
        {
        ...
        //Switch Containerを設定
        private void Add_CAkSwitchCntr(BNK_Relation Temp_Relation, int From_Line, int To_Line)
        {
                        ...
                        foreach (Switch_Relation Switch_Parent in Switch.Switch_Info)
                        {
                            ...
                            foreach (Switch_Child_Relation Switch_Child in Switch_Parent.Children)
                            {
                                if (Switch_Child.ShortID == Child_Short_ID)
                                {
                                    ...
                                    Switch_Child_Container.Add(new Switch_Child_Container(Switch_Child.GUID, Parent_ShortID, Child_Short_ID, Child_Containers));
                                    IsExist = true;
                                    break;
                                }
                            }
        ...
        //Soundを設定
        private void Add_CAkSound(BNK_Relation Temp_Relation, int From_Line, int To_Line, bool IsExtractSound)
        {
            ...
            for (int Number = From_Line; Number <= To_Line; Number++)
            {
                ...
                else if (BNK_Info.Read_All[Number].Contains("name=\"bIsLanguageSpecific\""))
                {
                    ...
                    GetCAkSoundLanguage = Value == "0" ? "SFX" : "ja";
                }
            }
            if (IsExtractSound)
            {
        ...
        private void Add_CAkCommon(BNK_Relation Temp_Relation, int From_Line, int To_Line, bool Include_Option, string Type_Name_Project, bool IsChildrenEndLine = true)
        {
            More_Class.List_Add(BNK_Info.Actor_Mixer_Hierarchy_Project, "<PropertyList>");
            More_Class.List_Add(BNK_Info.Actor_Mixer_Hierarchy_Project, "</PropertyList>");
        ...
        //初期値を記入
        private void Add_PropertyList(BNK_Relation Temp_Relation, string Name, string Value, Property_Type Type, bool IsIntMode, bool IsOneLine = false)
        {
            int Line_Start = -1;
            for (int Number = 5; Number < BNK_Info.Actor_Mixer_Hierarchy_Project.Count; Number++)
            {
        ...
        //範囲を記入
        private void Add_Property_Range(BNK_Relation Temp_Relation, string Name, string Min_Value, string Max_Value, Property_Type Type)
        {
            int Line_Start = -1;
            for (int Number = 5; Number < BNK_Info.Actor_Mixer_Hierarchy_Project.Count; Number++)
            {
        ...
        private void Add_Property_RTPC(BNK_Relation Temp_Relation, int RTPC_Start_Line, int LayerModeWriteLine = 0)
        {
            string GetRTPCParam = "";
        ...
        private void Add_Property_State(int Start_Line, int From_Line, int To_Line)
        {
            for (int Number = From_Line; Number <= To_Line; Number++)
            {
                string Read_Line = BNK_Info.Read_All[Number];
