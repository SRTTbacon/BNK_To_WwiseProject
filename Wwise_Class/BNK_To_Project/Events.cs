    ...
    public class Events
    {
        ...
        public static async Task Set_Name(System.Windows.Controls.TextBlock Message_T)
        {
            if (Event_ShortID.Count > 0)
            {
                foreach (uint ShortID_Now in Event_ShortID)
                {
                    ...
                    Message_T.Text = Message_T.Text = "Converting '" + ShortID_Now + "' to a string...";
                    await Task.Delay(50);
        ...
        public static void Create(string To_File)
        {
            ...
            List<string> Write_All = new List<string>
            {
                ...
                "<WwiseDocument Type=\"WorkUnit\" ID=\"{" + Event_Unit + "}\" SchemaVersion=\"97\">",
                "<Events>",
                "<WorkUnit Name=\"SRTTbacon\" ID=\"{" + Event_Unit + "}\" PersistMode=\"Standalone\">",
                "<ChildrenList>"
            };
        ...
        private static int Change_Name_To_Mode(string Name)
        {
            if (Name == "Play")
                return 1;
