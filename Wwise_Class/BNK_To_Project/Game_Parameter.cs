    ...
    public class Game_Parameter
    {
        ...
        public static void Create(string To_File)
        {
            ...
            for (int Number = 0; Number < BNK_Info.RTPC_Info.Count; Number++)
            {
                ...
                {
                    string Not_Use_Count_String = Not_Use_Count < 10 ? "0" + Not_Use_Count : Not_Use_Count.ToString();
            ...
            List<string> Write_Param = new List<string>
            {
                ...
                "<WwiseDocument Type=\"WorkUnit\" ID=\"{" + BNK_Info.Parent_RTPC_WorkUnit + "}\" SchemaVersion=\"97\">",
                "<GameParameters>",
                "<WorkUnit Name=\"SRTTbacon\" ID=\"{" + BNK_Info.Parent_RTPC_WorkUnit + "}\" PersistMode=\"Standalone\">",
                "<ChildrenList>"
            };
            //RTPCの数だけループ
        ...
        private static int Get_Param_Index_From_Name(string Name)
        {
            if (Name == "None")
                return 0;
        ...
        private static int Get_Mode_Index_From_Name(string Name)
        {
            if (Name == "SlewRate")
                return 1;
