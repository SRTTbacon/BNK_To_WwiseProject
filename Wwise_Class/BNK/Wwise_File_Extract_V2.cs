...
namespace WoTB_Voice_Mod_Creater.Wwise_Class
{
    ...
    public class Wwise_File_Extract_V2
    {
        ...
        {
            return WEML.Count <= Index || IsClear ? "" : WEML[Index].ID.ToString();
        }
        //.bnk内のファイル数を取得
        ...
        public bool Wwise_Extract_To_WEM_Directory_V2(string To_Dir)
        {
            ...
            try
            {
                if (!Directory.Exists(To_Dir))
                    _ = Directory.CreateDirectory(To_Dir);
                StreamWriter stw = File.CreateText(Sub_Code.Special_Path + "/Wwise_Parse/BNK_Extract.bat");
                stw.WriteLine("chcp 65001");
                stw.Write(Sub_Code.Special_Path + "/Wwise_Parse/BNK_Extract.exe -a \"" + Selected_BNK_File + "\" -o \"" + To_Dir + "\" --oggs-only");
                stw.Close();
                ProcessStartInfo processStartInfo = new ProcessStartInfo
                {
                    FileName = Sub_Code.Special_Path + "/Wwise_Parse/BNK_Extract.bat",
                    CreateNoWindow = true,
                    UseShellExecute = false
                };
                Process p = Process.Start(processStartInfo);
                p.WaitForExit();
                File.Delete(Sub_Code.Special_Path + "/Wwise_Parse/BNK_Extract.bat");
                return true;
            }
            catch
            {
        ...
        public bool Wwise_Extract_To_Ogg_Directory(string To_Dir, int Name_Mode, ref List<string> File_Names)
        {
            ...
            try
            {
                ...
                if (Wwise_Extract_To_WEM_Directory(To_Dir, Name_Mode, ref Temp))
                {
                    ...
                    {
                        _ = Sub_Code.WEM_To_File(File_Now, File_Now.Replace(".wem", ".ogg"), "ogg", true);
        ...
        public void Set_Sound_Name(string SoundbanksInfo, string Dir)
        {
            ...
            for (int Number = 0; Number < Lines.Length; Number++)
            {
                if (Lines[Number].Contains("\"Id\": \"") && Lines[Number + 2].Contains("\"ShortName\": \""))
                {
                    ...
                    _ = Sub_Code.File_Move_V2(Dir + "\\" + Temp_02, Dir + "\\" + Temp_05, true);
                    Number += 2;
                }
            }
        }
