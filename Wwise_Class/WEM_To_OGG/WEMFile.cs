...
namespace WoTB_Voice_Mod_Creater.Wwise_Class
{
    public class WEM_To_OGG
    {
        public static bool Create_OGG(string WEM_File, string To_OGG_File)
        {
            ...
            try
            {
                wem.GenerateOGG(To_OGG_File, BNK_To_WwiseProject.Class.Sub_Code.Special_Path + "\\Wwise\\packed_codebooks_aoTuV_603.bin", false, false);
                Return = true;
            }
            catch { }
            wem.Close();
    ...
    public class WEMFile
    {
        ...
        public WEMFile(string fileLocation, WEMForcePacketFormat forcePacketFormat)
        {
            ...
            using (BinaryReader br = new BinaryReader(_wemFile, Encoding.UTF8, true))
            {
                ...
                _ = br.BaseStream.Seek(_fmtChunkOffset, SeekOrigin.Begin);
                ...
                {
                    _ = br.BaseStream.Seek(_cueChunkOffset, SeekOrigin.Begin);
                    _cueCount = br.ReadUInt32();
                }
                if (_smplChunkOffset != 0xFFFFFFFF)
                {
                    _ = br.BaseStream.Seek(_smplChunkOffset, SeekOrigin.Begin);
                    _loopCount = br.ReadUInt32();
                    if (_loopCount != 1)
                        throw new Exception("SMPL Chunk - ループカウントを取得できませんでした。");
                    _ = br.BaseStream.Seek(_smplChunkOffset + 0x2C, SeekOrigin.Begin);
                ...
                switch (_vorbChunkSize)
                {
                    ...
                    case 0x32:
                    case 0x34:
                        _ = br.BaseStream.Seek(_vorbChunkOffset, SeekOrigin.Begin);
                        break;
                    default:
                ...
                switch (_vorbChunkSize)
                {
                    ...
                    case 0x2A:
                        {
                            _noGranule = true;
                            _ = br.BaseStream.Seek(_vorbChunkOffset + 4, SeekOrigin.Begin);
                            uint modSignal = br.ReadUInt32();
                            if (modSignal != 0x4A && modSignal != 0x4B && modSignal != 0x69 && modSignal != 0x70)
                                _modPackets = true;
                            _ = br.BaseStream.Seek(_vorbChunkOffset + 0x10, SeekOrigin.Begin);
                            break;
                        }
                    default:
                        _ = br.BaseStream.Seek(_vorbChunkOffset + 0x18, SeekOrigin.Begin);
                        break;
                }
                ...
                switch (_vorbChunkSize)
                {
                    case 0xFFFFFFFF:
                    case 0x2A:
                        _ = br.BaseStream.Seek(_vorbChunkOffset + 0x24, SeekOrigin.Begin);
                        break;
                    case 0x32:
                    case 0x34:
                        _ = br.BaseStream.Seek(_vorbChunkOffset + 0x2C, SeekOrigin.Begin);
                        break;
                }
                switch (_vorbChunkSize)
                {
        ...
        public void GenerateOGG(string fileLocation, string codebooksLocation, bool inlineCodebooks, bool fullSetup)
        {
            ...
            using (OggStream ogg = new OggStream(File.Create(fileLocation)))
            {
                ...
                while (offset < _dataChunkOffset + _dataChunkSize)
                {
                    ...
                    _ = _wemFile.Seek(offset, SeekOrigin.Begin);
                    ...
                    if (_modPackets)
                    {
                        ...
                        {
                            _ = _wemFile.Seek(nextOffset, SeekOrigin.Begin);
                            ...
                            if (nextOffset + packetHeaderSize <= _dataChunkOffset + _dataChunkSize)
                            {
                                ...
                                {
                                    _ = _wemFile.Seek(audioPacket.GetOffset(), SeekOrigin.Begin);
                                    BitStream bitStream = new BitStream(_wemFile);
                            ...
                            _ = _wemFile.Seek(offset + 1, SeekOrigin.Begin);
                        }
        ...
        private void GenerateOGGHeaderTriad(OggStream ogg)
        {
            ...
            _ = _wemFile.Seek(informationPacket.GetOffset(), SeekOrigin.Begin);
            byte[] informationPacketType = new byte[1];
            _ = _wemFile.Read(informationPacketType, 0, 1);
            ...
            if (commentPacket.GetGranule() != 0)
                throw new Exception("ヘッダーの生成中にエラーが発生しました。");
            _ = _wemFile.Seek(commentPacket.GetOffset(), SeekOrigin.Begin);
            byte[] commentPacketType = new byte[1];
            _ = _wemFile.Read(commentPacketType, 0, 1);
