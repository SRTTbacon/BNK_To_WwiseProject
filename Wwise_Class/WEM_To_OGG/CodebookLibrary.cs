...
namespace WEMSharp
{
    internal class CodebookLibrary
    {
        ...
        internal CodebookLibrary(string fileLocation)
        {
            ...
            {
                _ = br.BaseStream.Seek(br.BaseStream.Length - 4, SeekOrigin.Begin);
        ...
        {
            return _codebookData == null || _codebookOffsets == null
                ? throw new Exception("コードブックを初期化できませんでした。")
                : index >= CodebookCount - 1
                ? null
                : _codebookData.ToList().GetRange((int)_codebookOffsets[index], (int)GetCodebookSize(index)).ToArray();
        }
        internal uint GetCodebookSize(uint index)
        {
            return _codebookData == null || _codebookOffsets == null
                ? throw new Exception("コードブックを初期化できませんでした。")
                : index >= CodebookCount - 1 ? 0xFFFFFFFF : _codebookOffsets[index + 1] - _codebookOffsets[index];
        }
