...
namespace WoTB_Voice_Mod_Creater.Wwise_Class
{
    ...
    {
        private const string Vorbis_Quality_High_ID = "53A9DE0F-3F4F-4B59-8614-3F9E3C7358FC";
        private const string Vorbis_Quality_High_WorkID = "F6B2880C-85E5-47FA-A126-645B5DFD9ACC";
        private const string Master_Audio_Bus_ID = "1514A4D8-1DA6-412A-A17E-75CA0C2149F3";
        private const string Master_Audio_Bus_WorkID = "005C6247-5812-4D7E-86EA-2F3C50B5E166";
        public string Project_Dir { get; private set; }
        private readonly List<string> Actor_Mixer_Hierarchy = new List<string>();
        private readonly List<string> Add_Wav_Files = new List<string>();
        private readonly List<string> Add_Other_Files = new List<string>();
        private readonly List<string> Add_All_Files = new List<string>();
        private readonly List<string> Delete_FIles = new List<string>();
        private readonly List<string> Add_WEM_Files = new List<string>();
        private readonly List<string> Delete_CAkSound_List = new List<string>();
        private readonly List<Music_Play_Time> Add_All_Files_Time = new List<Music_Play_Time>();
        ...
        public void Add_Sound(string Container_ShortID, string Audio_File, string Language, bool IsSetShortIDMode = false, Music_Play_Time Time = null, string Effect = "", int Set_Volume = 0, bool IsDeleteCAkSound = false, bool IsUseHash = true, bool IsLoop = false)
        {
            if (Project_Dir == "")
                return;
            try
            {
                string FileName_Short_ID = IsUseHash ? WwiseHash.HashString(Audio_File + Container_ShortID).ToString() : Path.GetFileNameWithoutExtension(Audio_File);
                if (Language == "SFX")
                {
                ...
                else if (ChildrenList_Line == 0)
                {
                    ...
                    More_Class.List_Add(Actor_Mixer_Hierarchy, "</ChildrenList>");
                    if (IsSetShortIDMode)
                        List_Add_File(ReferenceListEnd_Line + 2, FileName_Short_ID + ".wav", Language, uint.Parse(FileName_Short_ID), Effect, Set_Volume, IsLoop);
                    else
                        List_Add_File(ReferenceListEnd_Line + 2, FileName_Short_ID + ".wav", Language, 0, Effect, Set_Volume, IsLoop);
                }
                else
                {
                    if (IsSetShortIDMode)
                        List_Add_File(ChildrenList_Line + 1, FileName_Short_ID + ".wav", Language, uint.Parse(FileName_Short_ID), Effect, Set_Volume, IsLoop);
                    else
                        List_Add_File(ChildrenList_Line + 1, FileName_Short_ID + ".wav", Language, 0, Effect, Set_Volume, IsLoop);
                }
            }
            catch
            {
        ...
        public void Project_Build(string BankName, string OutputFilePath, string GeneratedSoundBanksPath = null, bool IsUseCache = false)
        {
            ...
            try
            {
                ...
                StreamWriter stw = File.CreateText(Sub_Code.Special_Path + "/Wwise/Project_Build.bat");
                stw.WriteLine("chcp 65001");
                stw.Write("\"" + Sub_Code.Special_Path + "/Wwise/x64/Release/bin/WwiseCLI.exe\" \"" + Project_File + "\" -GenerateSoundBanks -Language ja -Platform Windows ");
                if (IsUseCache)
                ...
                {
                    FileName = Sub_Code.Special_Path + "/Wwise/Project_Build.bat",
                    CreateNoWindow = true,
                    UseShellExecute = false
                };
                Process p = Process.Start(processStartInfo);
                p.WaitForExit();
                File.Delete(Sub_Code.Special_Path + "/Wwise/Project_Build.bat");
                string GeneratedFile = GeneratedSoundBanksPath == null
                    ? Directory.GetFiles(Project_Dir + "/GeneratedSoundBanks/Windows", BankName + ".bnk", SearchOption.AllDirectories)[0]
                    : Directory.GetFiles(Project_Dir + "/GeneratedSoundBanks/" + GeneratedSoundBanksPath, BankName + ".bnk", SearchOption.AllDirectories)[0];
                _ = Sub_Code.File_Move(GeneratedFile, OutputFilePath, true);
            }
            catch (Exception e)
            {
                Sub_Code.Error_Log_Write(e.Message);
        ...
        public void Clear(string CachePath = null, bool IsOnlyAddFile = false)
        {
            try
            {
                ...
                foreach (string File_Now in Delete_FIles)
                    Sub_Code.File_Delete_V2(File_Now);
                string[] GetWEMFiles = CachePath == null
                    ? Directory.GetFiles(Project_Dir + "/.cache/Windows", "*.wem", SearchOption.AllDirectories)
                    : Directory.GetFiles(Project_Dir + "/.cache/" + CachePath, "*.wem", SearchOption.AllDirectories);
                if (IsOnlyAddFile)
                {
        ...
        private void List_Add_File(int Line_Number, string FileName, string Language, uint Set_Media_ID = 0, string Effect = "", int Set_Volume = 0, bool IsLoop = false)
        {
            try
            {
        ...
        public void Sound_Music_Add_Wwise(string Dir_Name)
        {
            ...
            {
                string Name_Only = Path.GetFileNameWithoutExtension(Voice_Now).Contains("reload")
                    ? Voice_Mod_Create.Get_Voice_Type_V2(Voice_Now)
                    : Voice_Mod_Create.Get_Voice_Type_V1(Voice_Now);
                switch (Name_Only)
                {
                    case "battle_bgm":
        ...
        public void Loading_Music_Add_Wwise(string Music_File, int Music_Index, Music_Play_Time Time, bool IsFeed_In_Mode, int Set_Volume, int Page = 0)
        {
            ...
            if (Music_Index == 12 || Music_Index == 14 || Music_Index == 16)
            {
                ...
                Music_File = Sub_Code.Special_Path + "\\Encode_Mp3\\Temp_" + Sub_Code.Generate_Random_String(2, 6) + ".wav";
                Delete_FIles.Add(Music_File);
                _ = Sub_Code.Audio_Encode_To_Other(From_File, Music_File, "wav", false);
                Sub_Code.Volume_Set_Start(Music_File, Sub_Code.Encode_Mode.WAV);
            }
            if (Page == 0)
            {
                if (Music_Index == 0)
        ...
        public void Event_Reset()
        {
            foreach (string Name_Now in Directory.GetFiles(Project_Dir + "/Events", "*.bak", SearchOption.TopDirectoryOnly))
            {
                ...
                _ = Sub_Code.File_Move(Name_Now, To_File, true);
            }
        }
    }
}
public class More_Class
{
    ...
    private static int Init_Line = 0;
    //指定した行に文字を挿入
...
public class WwiseHash
{
    ...
    private static uint FnvHash(byte[] input, bool use32bits)
    {
        uint prime = 16777619;
        ...
        return use32bits ? hash : (hash >> 30) ^ (hash & mask);
    }
}