...
namespace WoTB_Voice_Mod_Creater.Wwise_Class
{
    ...
    public class Wwise_File_Extract_V1
    {
        ...
        private bool IsPCKSelected = false;
        private Wwise_Pack Wwise;
        private readonly List<Wwise_Pack.SoundFile> Sounds = new List<Wwise_Pack.SoundFile>();
        public Wwise_File_Extract_V1(string PCK_File)
        {
            if (!File.Exists(PCK_File))
                return;
            Selected_PCK_File = PCK_File;
            ...
            try
            {
                ...
                for (int Number = 0; Number < Sounds.Count; Number++)
                {
                    ...
                    if (Wwise_Extract_To_WEM_File(Number, To_Dir + "\\" + Number_01 + ".wem", true))
                        _ = Sub_Code.WEM_To_File(To_Dir + "\\" + Number_01 + ".wem", To_Dir + "\\" + Number_01 + ".wav", "wav", true);
                }
                return true;
            }
            catch (Exception e)
            ...
            try
            {
                ...
                for (int Number = 0; Number < Sounds.Count; Number++)
                {
                    ...
                    if (IsCountUpMode)
                        Number_01 = Number + 1;
                    if (Wwise_Extract_To_WEM_File(Number, To_Dir + "\\" + Number_01 + ".wem", true))
                        _ = Sub_Code.WEM_To_OGG_WAV(To_Dir + "\\" + Number_01 + ".wem", To_Dir + "\\" + Number_01, true);
                }
                return true;
            }
            catch (Exception e)
        ...
        private async Task Async_Wwise_Extract_To_WEM_File(int Index, string To_Dir)
        {
            try
            {
                await Task.Run(() =>
        ...
        private async Task Async_Wwise_Extract_To_Ogg_File(int Index, string To_Dir)
        {
            await Task.Run(() =>
            {
                try
                {
                    ...
                    wem.GenerateOGG(To_Dir + "/" + Sounds[Index].id + ".ogg", Sub_Code.Special_Path + "/Wwise/packed_codebooks_aoTuV_603.bin", false, false);
                    wem.Close();
                }
                catch
                {
        ...
        public void Wwise_PCK_Save(string To_File, string Set_Dir, bool IsOverWrite)
        {
            ...
            try
            {
                StreamWriter stw = File.CreateText(Sub_Code.Special_Path + "/Wwise/Wwise_PCK_Repack.bat");
                stw.WriteLine("chcp 65001");
                if (Selected_PCK_File == To_File)
                    stw.Write("\"" + Sub_Code.Special_Path + "/Wwise/wwiseutil.exe\" -f \"" + Selected_PCK_File + "\" -r -t \"" + Set_Dir + "\"");
                else
                    stw.Write("\"" + Sub_Code.Special_Path + "/Wwise/wwiseutil.exe\" -f \"" + Selected_PCK_File + "\" -o \"" + To_File + "\" -r -t \"" + Set_Dir + "\"");
                stw.Close();
                ProcessStartInfo processStartInfo = new ProcessStartInfo
                {
                    FileName = Sub_Code.Special_Path + "/Wwise/Wwise_PCK_Repack.bat",
                    CreateNoWindow = true,
                    UseShellExecute = false
                };
                Process p = Process.Start(processStartInfo);
                p.WaitForExit();
                File.Delete(Sub_Code.Special_Path + "/Wwise/Wwise_PCK_Repack.bat");
            }
            catch (Exception e)
            {
                Sub_Code.Error_Log_Write(e.Message);
